<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="新闻标题" prop="xinwenbiaoti">
          <el-input v-model="ruleForm.xinwenbiaoti" 
              placeholder="新闻标题" clearable  :readonly="ro.xinwenbiaoti"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="新闻标题" prop="xinwenbiaoti">
              <el-input v-model="ruleForm.xinwenbiaoti" 
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="文章来源" prop="wenzhanglaiyuan">
          <el-input v-model="ruleForm.wenzhanglaiyuan" 
              placeholder="文章来源" clearable  :readonly="ro.wenzhanglaiyuan"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="文章来源" prop="wenzhanglaiyuan">
              <el-input v-model="ruleForm.wenzhanglaiyuan" 
                placeholder="文章来源" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="新闻类型" prop="xinwenleixing">
          <el-select :disabled="ro.xinwenleixing" v-model="ruleForm.xinwenleixing" placeholder="请选择新闻类型">
            <el-option
                v-for="(item,index) in xinwenleixingOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="新闻类型" prop="xinwenleixing">
	      <el-input v-model="ruleForm.xinwenleixing"
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.xinwentupian" label="新闻图片" prop="xinwentupian">
          <file-upload
          tip="点击上传新闻图片"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.xinwentupian?ruleForm.xinwentupian:''"
          @change="xinwentupianUploadChange"
          ></file-upload>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.xinwentupian" label="新闻图片" prop="xinwentupian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.xinwentupian.split(',')" :src="$base.url+item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="新闻内容" prop="xinwenneirong">
                <editor 
              ...
              <div v-else>
                <el-form-item v-if="ruleForm.xinwenneirong" label="新闻内容" prop="xinwenneirong">
                    <span v-html="ruleForm.xinwenneirong"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.wenzhanglaiyuan = true;
          ...
          if(o=='xinwenleixing'){
            this.ruleForm.xinwenleixing = obj[o];
	    this.ro.xinwenleixing = true;
            continue;
          }
          if(o=='fabushijian'){
	    ...
	    this.ro.xinwenneirong = true;
          ...
          if(o=='xinwentupian'){
            this.ruleForm.xinwentupian = obj[o];
	    this.ro.xinwentupian = true;
            continue;
          }
          if(o=='faburen'){
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/xinwenleixing/xinwenleixing`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.xinwenleixingOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
	...
	this.ruleForm.xinwenneirong = this.ruleForm.xinwenneirong.replace(reg,'../../../ssmf495f/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
	...
	if(this.ruleForm.xinwentupian!=null) {
		this.ruleForm.xinwentupian = this.ruleForm.xinwentupian.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `falvxinwen/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.falvxinwenCrossAddOrUpdateFlag = false;
						 this.parent.search();
    ...
    xinwentupianUploadChange(fileUrls) {
	this.ruleForm.xinwentupian = fileUrls;
	this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
