<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info'&& !ro.shipinjiangjie" label="视频讲解" prop="shipinjiangjie">
          <file-upload
          tip="点击上传视频讲解"
          action="file/upload"
          :limit="1"
          :multiple="true"
          :fileUrls="ruleForm.shipinjiangjie?ruleForm.shipinjiangjie:''"
          @change="shipinjiangjieUploadChange"
          ></file-upload>
        </el-form-item> 
        <div v-else>
          <el-form-item v-if="ruleForm.shipinjiangjie" label="视频讲解" prop="shipinjiangjie">
            <el-button type="text" size="small" @click="download(ruleForm.shipinjiangjie)">预览</el-button>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="法律内容" prop="falvneirong">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.falvneirong" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.falvneirong" label="法律内容" prop="falvneirong">
                    <span v-html="ruleForm.falvneirong"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.zhangjie = true;
          ...
          if(o=='shipinjiangjie'){
            this.ruleForm.shipinjiangjie = obj[o];
	    this.ro.shipinjiangjie = true;
            continue;
          }
          if(o=='falvneirong'){
            this.ruleForm.falvneirong = obj[o];
	    this.ro.falvneirong = true;
            continue;
          }
          if(o=='fabushijian'){
	...
	this.ruleForm.falvneirong = this.ruleForm.falvneirong.replace(reg,'../../../ssmf495f/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
	...
	if(this.ruleForm.shipinjiangjie!=null) {
		this.ruleForm.shipinjiangjie = this.ruleForm.shipinjiangjie.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `falvzhishi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.falvzhishiCrossAddOrUpdateFlag = false;
						 this.parent.search();
    ...
    shipinjiangjieUploadChange(fileUrls) {
	this.ruleForm.shipinjiangjie = fileUrls;
	this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
