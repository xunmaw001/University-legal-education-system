<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="律师等级" prop="lvshidengji">
          <el-input v-model="ruleForm.lvshidengji" 
              placeholder="律师等级" clearable  :readonly="ro.lvshidengji"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="律师等级" prop="lvshidengji">
              <el-input v-model="ruleForm.lvshidengji" 
                placeholder="律师等级" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
...
export default {
  data() {
	...
	userid : false,
      ...
      rules: {
          lvshizhanghao: [
                { required: true, message: '律师账号不能为空', trigger: 'blur' },
          ],
          lvshixingming: [
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.shenfenzheng = true;
          ...
          if(o=='lvshidengji'){
            this.ruleForm.lvshidengji = obj[o];
	    this.ro.lvshidengji = true;
            continue;
          }
          if(o=='gerenjianjie'){
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          var json = data.data;
		if(json.lvshizhanghao!=''&&json.lvshizhanghao){
                this.ruleForm.lvshizhanghao = json.lvshizhanghao
	    		this.ro.lvshizhanghao = true;
		}
		if(json.lvshixingming!=''&&json.lvshixingming){
		...
		if(json.lvshidengji!=''&&json.lvshidengji){
                this.ruleForm.lvshidengji = json.lvshidengji
	    		this.ro.lvshidengji = true;
		}
        } else {
	...
	this.ruleForm.gerenjianjie = this.ruleForm.gerenjianjie.replace(reg,'../../../ssmf495f/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `lvshituijian/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.lvshituijianCrossAddOrUpdateFlag = false;
						 this.parent.search();
