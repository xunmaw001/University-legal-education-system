...
export default {
  ...
  methods: {
	...
	this.ruleForm.huifuneirong = this.ruleForm.huifuneirong.replace(reg,'../../../ssmf495f/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `wentihuifu/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.wentihuifuCrossAddOrUpdateFlag = false;
						 this.parent.search();
